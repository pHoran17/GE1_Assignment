...
public class EdgeCube : MonoBehaviour
{
    ...
    void Update()
    {
        if (useBuffer)
        {
            ...
            transform.localScale = new Vector3(transform.localScale.x, audioH.bandBuffer[band] * startScale, transform.localScale.z);
        }
        if (!useBuffer)
        {

            transform.localScale = new Vector3(transform.localScale.x, audioH.spectrum[band] * startScale, transform.localScale.z);
